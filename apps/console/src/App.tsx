...
function SupervisorView({ token, user, conversations, agents, summary, refresh }: { token: string; user: User; conversations: Conversation[]; agents: AgentRow[]; summary: { conversations: Record<string, number>; agents: Record<string, number> } | null; refresh: () => void }) {
  ...
  return <div className="page-content">
    <header className="page-header"><div><span className="section-kicker">OPERATIONS COMMAND</span><h1>Good afternoon, {user.display_name.split(" ")[0]}</h1><p>Live performance across your contact centre.</p></div><div className="header-actions"><span className="live-button"><i /> Live</span></div></header>
...
function ConfigurationView({ token, admin }: { token: string; admin: boolean }) {
  ...
  async function savePilot(event: FormEvent) { event.preventDefault(); await api.updatePilot(token, { campaign_name: campaignName, queue_name: queueName, script_content: scriptContent, required_steps: ["Confirm customer identity", "Acknowledge the issue", "Recap the resolution"], knowledge_title: knowledgeTitle, knowledge_content: knowledgeContent, qa_form_name: qaName }); setSaved("Pilot configuration saved and audited."); await load(); }
  ...
  const loadConversation = useCallback(async () => {
    ...
    try {
      ...
      if (selected.channel === "web_chat") {
        ...
        if (call.session.state === "ended" && recordingRef.current?.id !== selectedId) {
