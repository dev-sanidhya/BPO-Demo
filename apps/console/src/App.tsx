...
function QualityView({ token, canReview }: { token: string; canReview: boolean }) {
  const [evaluations, setEvaluations] = useState<QAEvaluation[]>([]); const [selected, setSelected] = useState(""); const [detail, setDetail] = useState<Record<string, unknown> | null>(null); const [score, setScore] = useState(90); const [reason, setReason] = useState("Reviewed call recording and conversation notes."); const [campaignId, setCampaignId] = useState(""); const [agentId, setAgentId] = useState(""); const [fatalOnly, setFatalOnly] = useState(false); const [sortBy, setSortBy] = useState("created_at"); const [campaigns, setCampaigns] = useState<Array<Record<string, unknown>>>([]); const [agents, setAgents] = useState<AgentRow[]>([]); const [recordingUrl, setRecordingUrl] = useState(""); const audioRef = useRef<HTMLAudioElement>(null);
  const load = useCallback(async () => { const rows = await api.qaEvaluations(token, { campaign_id: campaignId, agent_id: agentId, fatal_only: fatalOnly, sort_by: sortBy }); setEvaluations(rows); setSelected((value) => rows.some((item) => item.id === value) ? value : rows[0]?.id || ""); }, [token, campaignId, agentId, fatalOnly, sortBy]);
  useEffect(() => { void load(); }, [load]); useEffect(() => { void Promise.all([api.configuration(token), api.agents(token)]).then(([config, agentRows]) => { setCampaigns((config.campaigns || []) as Array<Record<string, unknown>>); setAgents(agentRows); }); }, [token]); useEffect(() => { if (!selected) return; let url = ""; void (async () => { const nextDetail = await api.qaDetail(token, selected); setDetail(nextDetail); try { url = URL.createObjectURL(await api.recording(token, String(nextDetail.conversation_id))); setRecordingUrl(url); } catch { setRecordingUrl(""); } })(); return () => { if (url) URL.revokeObjectURL(url); }; }, [selected, token]);
  async function review(event: FormEvent) { event.preventDefault(); const resolution = detail?.fatal_triggered ? (detail.effective_fatal ? "confirmed" : "cleared") : undefined; await api.reviewQA(token, selected, score, reason, resolution); await load(); setDetail(await api.qaDetail(token, selected)); }
  function playEvidence(startMs: number) { if (audioRef.current) { audioRef.current.currentTime = startMs / 1000; void audioRef.current.play(); } }
  const answers = (detail?.answers as Array<Record<string, unknown>> | undefined) || [];
  const detailFatal = detail?.effective_fatal === true; const automaticFatal = detail?.fatal_triggered === true;
  return <div className="page-content"><header className="page-header"><div><span className="section-kicker">EVIDENCE-LED QUALITY</span><h1>Quality review</h1><p>Automatic scoring stays immutable; reviewers resolve fatal findings explicitly.</p></div></header><section className="report-filters"><label>Campaign<select aria-label="QA campaign" value={campaignId} onChange={(event) => setCampaignId(event.target.value)}><option value="">All campaigns</option>{campaigns.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></label><label>Agent<select aria-label="QA agent" value={agentId} onChange={(event) => setAgentId(event.target.value)}><option value="">All agents</option>{agents.map((item) => <option key={item.id} value={item.id}>{item.display_name}</option>)}</select></label><label>Sort<select aria-label="QA sort" value={sortBy} onChange={(event) => setSortBy(event.target.value)}><option value="created_at">Newest first</option><option value="score">Highest score first</option></select></label><label><span>Fatal only</span><input aria-label="Fatal QA only" type="checkbox" checked={fatalOnly} onChange={(event) => setFatalOnly(event.target.checked)} /></label></section><section className="quality-grid"><article className="panel score-list">{evaluations.map((item) => <button className={selected === item.id ? "selected" : ""} onClick={() => setSelected(item.id)} key={item.id}><div className="score-ring">{item.effective_score}</div><span><strong>{item.effective_fatal ? "Fatal finding" : item.status.startsWith("reviewed") ? "Human reviewed" : "Automatic evaluation"}</strong><small>{item.campaign} · {item.agent} · {formatTime(item.created_at)}</small></span><ChevronRight size={17} /></button>)}</article><article className="panel qa-detail"><div className="panel-head"><div><h3>Evaluation evidence</h3><p>Automatic: {String(detail?.automatic_score ?? "—")} {detailFatal ? "· Fatal finding" : automaticFatal ? "· Fatal cleared by reviewer" : ""}</p></div><ClipboardCheck size={18} /></div>{recordingUrl && <audio ref={audioRef} className="qa-recording" aria-label="QA call recording" controls src={recordingUrl} />}<div className="qa-answers">{answers.map((answer) => <button type="button" onClick={() => playEvidence(Number(answer.evidence_start_ms))} key={String(answer.id)}><span className={answer.passed ? "pass" : "fail"}>{answer.passed ? "PASS" : "FAIL"}</span><strong>{String(answer.question)}</strong><p>“{String(answer.evidence_quote)}”</p><small>{String(answer.evidence_start_ms)}–{String(answer.evidence_end_ms)} ms · confidence {String(answer.confidence)}% · {String(answer.reasoning)}</small></button>)}</div>{canReview && selected && <form className="review-form" onSubmit={review}><label>Reviewed score<input aria-label="Reviewed score" type="number" min="0" max="100" value={score} onChange={(event) => setScore(Number(event.target.value))} /></label>{automaticFatal && <label>Fatal finding<select aria-label="Fatal finding resolution" value={detailFatal ? "confirmed" : "cleared"} onChange={(event) => setDetail((current) => current ? { ...current, effective_fatal: event.target.value === "confirmed" } : current)}><option value="confirmed">Confirm fatal</option><option value="cleared">Clear fatal</option></select></label>}<label>Reason<input aria-label="Review reason" value={reason} onChange={(event) => setReason(event.target.value)} /></label><button>Save review</button></form>}</article></section></div>;
}
...
function ReportsView({ token, user }: { token: string; user: User }) {
  ...
  const trend = (summary.trend || []) as Array<Record<string, unknown>>; const reportAgents: Array<Record<string, unknown>> = ((summary.agents || []) as Array<Record<string, unknown>>).map((item) => ({ ...item, agent: `${String(item.agent)} · ${item.calls_per_active_hour == null ? "No completed voice time" : `${String(item.calls_per_active_hour)} calls/active hr`} · AHT ${item.voice_average_handle_seconds == null ? "—" : `${String(item.voice_average_handle_seconds)}s`}` })); const reportQueues = (summary.queues || []) as Array<Record<string, unknown>>;
