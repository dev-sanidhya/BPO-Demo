...
export const api = {
  ...
  qaEvaluations: (token: string, filters: Record<string, string | boolean> = {}) => { const params = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value) params.set(key, String(value)); }); return request<QAEvaluation[]>(`/qa/evaluations${params.size ? `?${params}` : ""}`, {}, token); },
  qaDetail: (token: string, id: string) => request<Record<string, unknown>>(`/qa/evaluations/${id}`, {}, token),
  reviewQA: (token: string, id: string, reviewedScore: number, reason: string, fatalResolution?: "confirmed" | "cleared") => request(`/qa/evaluations/${id}/reviews`, { method: "POST", body: JSON.stringify({ reviewed_score: reviewedScore, reason, fatal_resolution: fatalResolution }) }, token),
  searchIntelligence: (token: string, query: string) => request<Array<Record<string, unknown>>>(`/analytics/conversations/search?q=${encodeURIComponent(query)}`, {}, token),
  coachingActions: (token: string) => request<Array<Record<string, unknown>>>("/coaching/actions", {}, token),
  createCoachingAction: (token: string, payload: Record<string, unknown>) => request("/coaching/actions", { method: "POST", body: JSON.stringify(payload) }, token),
  acknowledgeCoachingAction: (token: string, id: string) => request(`/coaching/actions/${id}/acknowledge`, { method: "POST" }, token),
